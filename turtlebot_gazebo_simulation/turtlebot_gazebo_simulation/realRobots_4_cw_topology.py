...
class CW_Motion_Controller(Node):
    def __init__(self):
        ...
        for i in range(4):
            ...
            self.robot_odom_subscribers=self.create_subscription(Odometry, f'/{real_robot_ids[i]}/odom', self.create_odom_callback(i), 10)
    ...
    def p_controller(self):
        rate = 10
        for i in range(4):
            if self.robot_current_positions_x[i] is None or 0.0: #re + map/odom or odom subscribe?
                return
        ...
        if self.state == "Wait_signal":
            ...
            for i in range(4):
                ...
                if distance > self.distance_tolerance:
                    goal_orientation = math.atan2(dy, dx)
                    angle_error = self.normalize_angle(goal_orientation - self.robot_current_yaws[i])
                    twist.angular.z = max(min(self.kp_angular * angle_error, 0.3), -0.3)
                    twist.linear.x = max(min(self.kp_linear * distance, 0.25), 0.05)
                else:
                    twist.linear.x = 0.0
